...
export default function ContractorsList() {
    ...
    return (
        <div className='p-[10px] md:m-[10px]  md:w-auto '>
            {login === 2 && (
                <>
                    <div className='font-medium text-neutral-300 mb-1 border-1 rounded-sm p-[10px] flex justify-between login2'>
                                ...
                                {constructorsData
                                    ...
                                    .map((row, i) => (
                                        <TableRow key={row._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                            <TableCell align='right'>{((page - 1) * 15) + i + 1}</TableCell>
                                            <TableCell align='right' className="border border-slate-300"><AccountCircleIcon /> {row.SHEM_YESHUT}</TableCell>
                                            <TableCell align='right' className="border border-slate-300"> <Link onClick={() => window.location.href = `mailto:${row.EMAIL}`}>{row.EMAIL}</Link></TableCell>
                                            <TableCell align='right' className="border border-slate-300">{row.MISPAR_TEL}</TableCell>
                                            <TableCell align='right' className="border border-slate-300">{row.KABLAN_MUKAR}</TableCell>
                                            <TableCell align='right' className="border border-slate-300">{row.SHEM_YISHUV}</TableCell>
                                            <TableCell align='right' className="border border-slate-300">{row.SHEM_REHOV}</TableCell>
                                            <TableCell align='right' className="border border-slate-300">{row.TEUR_ANAF}</TableCell>
                                            <TableCell align='right' className="border border-slate-300">{row.OVDIM}</TableCell>
                                        </TableRow>
                                    ))}
                            </TableBody>
                        </Table>
